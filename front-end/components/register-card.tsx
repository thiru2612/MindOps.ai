...
export function RegisterCard({ onRegister, isAnimating }: RegisterCardProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token) {
        ...
        // Registration successful but no auto-login, still trigger animation
        setIsLoading(false);
        onRegister();
      }
    } catch (err) {
      ...
      <div className="relative rounded-2xl bg-slate-900/60 backdrop-blur-2xl border border-slate-700/50 shadow-2xl shadow-black/20 overflow-hidden">
        ...
        <div className="relative z-10 p-8 sm:p-10">
          ...
          <div className="text-center mb-8">
            ...
            >
              Join the MindOps Cloud Console
            </motion.p>
          </div>
          ...
          >
            <FieldGroup>
              <Field>
                <FieldLabel className="text-slate-300 text-sm">Full Name</FieldLabel>
          ...
          >
            ...
            <a href="/login" className="text-slate-300 hover:text-white transition-colors font-medium">
              Sign In
            </a>
          </motion.p>
        </div>
